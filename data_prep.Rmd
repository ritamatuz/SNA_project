...
#They seem to have a stupid whitespace after the country name that I don't have the patience to keep trying to remove right now
#world_fact_sheet$Country <- apply(world_fact_sheet$Country, FUN = lambda x: x.str.strip())
#world_fact_sheet.applymap(lambda x: x.strip() if isinstance(x, str) else x)
#unique(world_fact_sheet$Country)
...
```{r}
# Todo:
# - delete []s from country names and language names
# - filter for the countries we're using
# - (manually) go through the languages and select where it's unclear
```
...
```
FIRST ATTEMPT AT CREATING A NETWORK OBJECT
```{r}
# migration_net <- network::network(inflow_data$To, edgelist=)
# nevermind lol
```
